...
def build_finish_order(
    *,
    ...
    for driver_code, median_pos in aggregated["median_positions"].items():
        ...
        # The grid anchor exists because a grid slot proxies pace, which lets it damp
        # simulation noise. A steward's penalty voids that for the driver it moves: the
        # simulation already starts him from the penalised slot, so anchoring him there
        # again charges the penalty twice. The informative weight is therefore zero for
        # him, and untouched for everyone else.
        driver_anchor_weight = 0.0 if info.get("is_penalised") else fo_cfg.grid_anchor_weight
        ...
        # max_gain caps how far a driver may be reported ahead of his grid slot. It is
        # the same damping heuristic as the anchor, so it double-counts a penalty for the
        # same reason: the recovery it is capping is one the simulation already modelled
        # from the penalised slot. No artificial floor for a penalised driver.
        min_position_score = (
            1.0 if info.get("is_penalised") else max(1.0, reference_grid_pos - max_gain)
        )

        position_blend_score = (
            ((1.0 - driver_anchor_weight) * median_pos)
            + (driver_anchor_weight * reference_grid_pos)
            - racecraft_adjustment
        )
        ...
        for position_sample, grid_position_sample in zip(
            ...
            min_sample_position_score = (
                1.0
                if info.get("is_penalised")
                else max(1.0, float(grid_position_sample) - max_gain)
            )
            blended_position_samples.append(
                max(
                    (
                        ((1.0 - driver_anchor_weight) * position_sample)
                        + (driver_anchor_weight * float(grid_position_sample))
                        - racecraft_adjustment
                    ),
                    min_sample_position_score,
                )
