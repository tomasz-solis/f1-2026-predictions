...
def test_run_prediction_sprint_path_only_refreshes_sq_grid(patcher):
    """Sprint flow may refresh SQ, but it must not pull actual Q results into predictions."""
    mock_predictor = MagicMock()
...
def test_run_prediction_uses_explicit_year_for_fastf1_refresh(patcher):
    """The orchestration layer should pass the requested season to allowed refresh calls."""
    mock_predictor = MagicMock()
    mock_predictor.predict_qualifying.side_effect = [
        {"grid": [{"driver": "VER", "team": "Red Bull Racing", "position": 1}]},
        {"grid": [{"driver": "NOR", "team": "McLaren", "position": 2}]},
...
def test_run_prediction_keeps_predicted_race_input_confidence_when_q_incomplete(patcher):
    ...
    patcher.setattr(
        prediction_flow,
        "_resolve_current_checkpoint_session",
        lambda year, race_name, is_sprint: "Q",
    )
...
def test_run_prediction_uses_completed_q_results_for_post_q_race_input(patcher):
    ...
    def _fetch_actual(year: int, race_name: str, session_name: str):
        actual_calls.append(session_name)
        if session_name == "Q":
