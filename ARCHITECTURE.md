...
the season there is almost no signal beyond pre-season testing. By mid-season
...
The active production model is version `2.3`. This is a calibrated champion
update with target-split shadow challengers, not a wholesale model-family
promotion. Promotion decisions are target-specific and gated by replay evidence.
...
The production readiness gate writes `production_gate` to
`data/evaluation/2026_evaluation_report.json`. Shadow challengers are saved in
prediction artifacts and audited by target: main qualifying, Grand Prix race,
...
Race forecasts use historical overtaking difficulty to control grid anchoring,
movement floors, chaos, strategy sensitivity, and overtaking-skill effects.
